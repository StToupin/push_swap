...
static int		do_route(t_two_stacks *stacks, t_route route)
{
	while (route.a != 0)
		if (route.a * route.b > 0)
		{
			if (stacks_do_op(stacks, (route.a > 0) ? O_RR : O_RRR))
				return (1);
		...
		{
			if (stacks_do_op(stacks, (route.a > 0) ? O_RA : O_RRA))
				return (1);
			route.a += (route.a > 0) ? -1 : 1;
		}
	while (route.b != 0)
	{
		if (stacks_do_op(stacks, (route.b > 0) ? O_RB : O_RRB))
			return (1);
...
static void		push_swap(t_two_stacks *stacks)
{
	...
	do_route(stacks, route[best_route.index]);
	stacks_do_op(stacks, O_PB);
}

static void		push_back(t_two_stacks *stacks)
{
	t_index_value	max_b;
	int				i;
	t_cll_elem		*elem;
	...
	do_route(stacks, (t_route){0, dist_b, 0});
	stacks_do_op(stacks, O_PA);
}
